...
class CardUsageFileServiceImpl(
  ...
  override fun getCardUsageFileObject(cardId: Long, id: Long): FileObject {
    val entity = cardUsageFileOutPort.findOne(cardId, id) ?: throw HttpStatusCodeException.NotFound("카드 사용 내역 파일을 찾을 수 없습니다(cardId: $cardId, id: $id)")
    val metadata = fileOutPort.findFileMetadata(entity.fileKey) ?: throw HttpStatusCodeException.Conflict("파일 정보를 찾을 수 없습니다(fileKey: ${entity.fileKey})")
    return fileOutPort.findFileObject(metadata.id!!) ?: throw HttpStatusCodeException.NotFound("파일 정보를 찾을 수 없습니다(fileId: ${metadata.id})")
  }
